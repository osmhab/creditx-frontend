...
>
  <motion.div
    ...
    custom={i}
    className="max-w-6xl w-full flex flex-col md:flex-row items-center md:items-start gap-8 md:gap-12"
  >
    {/* ✅ Texte à gauche */}
    <div className="text-center md:text-left flex flex-col justify-center items-center md:items-start gap-6 max-w-xl">
      <h2 className="text-3xl md:text-5xl font-bold text-black">{slide.title}</h2>
      <p className="text-gray-700 text-base md:text-lg">{slide.text}</p>
      <Link
        to="/formulaire"
        className="bg-black text-white px-8 py-3 rounded-full text-base font-semibold hover:bg-gray-900 transition"
      >
    ...
    <img
      src={slide.image}
      alt={slide.title}
      className="w-40 h-40 md:w-[400px] lg:w-[480px] md:h-auto object-contain"

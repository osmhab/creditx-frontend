...
export default function InformationsPersonnelles() {
  ...
  const sousTitre = personne.role === "secondaire" ? "Demandeur 2" : "Demandeur principal et administrateur";

  return (
  <div className="min-h-screen bg-[#FCFCFC] flex justify-center px-4 pt-6">
    <div className="w-full max-w-md">

      <button onClick={() => navigate(`/informations-personnelles?id=${id}`)} className="text-xl mb-6">←</button>
      ...
      <div className="bg-white rounded-xl divide-y">
        {personne.role === "secondaire" &&
          renderLigne("Lien avec Habib Osmani", "relationAvecDemandeurPrincipal", null, `/informations/${index}/${id}/relation`)
        }
        ...
        {renderLigne(
          "Enfant(s) à charge",
          "enfantsACharge",
          personne.enfantsACharge === true ? "Oui" : personne.enfantsACharge === false ? "Non" : null,
